...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
	return (
		<motion.div
			...
			className={cn("relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-transparent py-2 lg:flex dark:bg-transparent", visible && "bg-white/80 dark:bg-neutral-950/80", className)}
		>
			{children}
		</motion.div>
	);
		...
		>
			{items.map((item, idx) => (
				<NavLink
					...
					onClick={onItemClick}
					className="relative px-4 py-2 text-white hover:text-neutral-600 duration-300 dark:text-neutral-300 text-base"
					key={`link-${idx}`}
					to={item.link}
				>
					{hovered === idx && (
